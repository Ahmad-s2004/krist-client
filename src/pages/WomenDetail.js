...
const WomenDetail = () => {
    ...
    let fetchData = async () => {
        let res = await fetch(`https://krist-server.vercel.app/product/getWomen/${_id}`)
        res = await res.json()
        setProduct(res)
        setLoading(true)
    }
                            ...
                            <div><span className='fw-semibold'>Size</span>: <span className='text-capitalize fw-semibold'>{data}</span>
                                <div>
                                    {product.size.map((x, index) => (
                                        <span key={index} className='text-uppercase me-2 btn btn-outline-dark rounded-4 my-2' onClick={() => handleSizeClick(x)} style={{ cursor: 'pointer' }}>{x}</span>
                                    ))}
                                </div>
                            </div>
                        }
